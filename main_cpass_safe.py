...
# =========================================================
# /CPASS - SAFE MICROSOFT PASSWORD-RECOVERY HELPER
# =========================================================

@bot.tree.command(
    name="cpass",
    description="Starts a manual Microsoft account password-recovery workflow."
)
@app_commands.describe(
    account="Microsoft account email",
    newpassword="New password you want to use",
    captchachannel="Channel where the recovery instructions will be posted"
)
async def cpass(
    interaction: discord.Interaction,
    account: str,
    newpassword: str,
    captchachannel: discord.TextChannel
):
    # This helper does not collect, process, or expose account passwords,
    # recovery codes, or CAPTCHA answers. Microsoft verification remains manual.
    if "@" not in account or len(account) > 320:
        await interaction.response.send_message(
            "❌ Please provide a valid Microsoft account email.",
            ephemeral=True
        )
        return
    ...
    embed = discord.Embed(
        title="🔐 Microsoft Password Recovery",
        description=(
            "Use Microsoft's official account-recovery process. "
            "Complete the CAPTCHA and verification code yourself."
        ),
        color=0x2B2D31
    )
    embed.add_field(name="**Email:**", value=account, inline=False)
    embed.add_field(name="**Old pass:**", value="`********`", inline=False)
    embed.add_field(name="**New pass:**", value="`********`", inline=False)
    embed.add_field(
        name="📌 Recovery",
        value=(
            "1. Open Microsoft's account recovery page.\n"
            "2. Complete the CAPTCHA manually.\n"
            "3. Enter the verification code sent by Microsoft.\n"
            "4. Finish the password reset on Microsoft's website."
        ),
        inline=False
    )
    embed.set_footer(text="Microsoft account recovery • Verification must be completed manually")

    try:
        await captchachannel.send(embed=embed)
        await interaction.response.send_message(
            f"✅ Recovery instructions sent to {captchachannel.mention}.",
            ephemeral=True
        )
    except discord.Forbidden:
        await interaction.response.send_message(
